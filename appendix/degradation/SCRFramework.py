# -*- coding: utf-8 -*-
"""
Created on Thu Jun  4 00:28:46 2020
...
class BRFramework:
    def __init__(self, XIN, FUL, LEVEL, B, W, ETA_C, ETA_D, MU):
        self.XIN = XIN
        self.FUL = FUL
        self.LEVEL = LEVEL
        self.B = B
    ...
    def Atheta_gmm(self,clf,T):
        ...
        theta[int(T-2)] = re/trunc*(self.ETA_C/self.ETA_D)*(1-self.MU*(T-t-1))
        while(t>0):
            ...
            theta[t] = (re1+re2)/trunc*(self.ETA_C/self.ETA_D)*(1-self.MU*(T-t-1))
        return theta
...
###############################################################################
    ...
    def general_ratio(self,D,P):
        ###############################pre data################################
        fold_i = 730; fold_n = 12-3; interval = 5
