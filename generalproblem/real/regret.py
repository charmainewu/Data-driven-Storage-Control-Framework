...
for BAR_POR,W in [[0.5,2],[1,2],[3,3]]:
    ...
    #costour,costrl,costmpc,costnos,costthb,costofl = BF.general_ratio(D,P)
    
    costour = np.load('./data/costour'+str(B)+'.npy')
    costrl = np.load('./data/costrl'+str(B)+'.npy')
    costmpc = np.load('./data/costmpc'+str(B)+'.npy')
    costnos = np.load('./data/costnos'+str(B)+'.npy')
    costthb = np.load('./data/costthb'+str(B)+'.npy')
    costofl = np.load('./data/costofl'+str(B)+'.npy')
    ...
    axs2.fill_between(x, min_ser, max_ser, alpha=0.15, color= 'lightsalmon', label = "Percentile 5%-95% for DETA" ) 
    ...
    axs2.fill_between(x, min_ser, max_ser, alpha=0.15, color= 'lightslategrey', label = "Percentile 5%-95% for RL" )
    ...
    axs2.fill_between(x, min_ser, max_ser, alpha=0.15, color = 'darkseagreen', label = "Percentile 5%-95% for MPC" )
    ...
    axs2.fill_between(x, min_ser, max_ser, alpha=0.15, color= 'wheat', label = "Percentile 5%-95% for THB" )
    ...
    axs2.fill_between(x, min_ser, max_ser, alpha=0.15, color = 'lightgray', label = "Percentile 5%-95% for NO STORAGE" )
    ...
    plt.savefig("./figure/rr"+str(B)+".pdf")
